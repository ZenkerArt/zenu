            ...
            for copy_curve in strip.fcurves:
                ...
                for index, key_active in enumerate(active_curve.keyframe_points):
                    strip.keyframe_insert(
                        data_path=copy_curve.data_path, frame=key_active.co.y)
...
class ZENU_PT_action_selector(BasePanel):
    ...
    def draw_settings(self, layout: bpy.types.UILayout):
        ...
        col = layout.column(align=True)
        op = col.operator(
            ZENU_OT_action_selector_item_operations.bl_idname, text='Select')
        op.action_name = action_active.action.name
        op.action = 'SELECT'

        op = col.operator(
            ZENU_OT_action_selector_item_operations.bl_idname, text='Rename To Action Name')

        op.action_name = action_active.action.name
        op.action = 'RENAME_TO_ACTION_NAME'

        op = col.operator(ZENU_OT_copy_nla_strip_settings.bl_idname)
        op.action_name = action_active.action.name
